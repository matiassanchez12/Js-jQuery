(async function load () {
  ...
  $form.addEventListener ('submit', async event => {
    ...
    try {
      const {data: {movies: pelis}} = await getData (
  ...
  function videoItemTemplate (movie) {
    const funcFind = element =>
      element === 'Drama' || element === 'Action' || element === 'Animation';

    let indexGenre = movie.genres.findIndex (funcFind);

    return `<div class="primaryPlaylistItem" data-id="${movie.id}" data-category="${movie.genres[indexGenre]}">
  ...
  //! Traigo datos random de una api que contiene informacion de un usuario
  //! para que aparezca en la seccion de "PLAYDEAMIGOS"
  ...
  //! Traigo el elemento del DOM con el nombre de clase que contiene los parametros
  const $playListFriends = document.getElementsByClassName (
    'playlistFriends-item'
  );

  //! Paso a la funcion setdataPersona el listado de personas que va a
  //! recibir datos nuevos.
  setDataPerson ($playListFriends);

  //! Evento para cuando se clickea en un amigo, se abra un modal
  ...
  //! Muestro un MODAL con el NOMBRE, FOTO Y EMAIL, de un amigo
